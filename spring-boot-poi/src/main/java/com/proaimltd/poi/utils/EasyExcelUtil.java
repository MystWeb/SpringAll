...
public class EasyExcelUtil {
    ...
    public static <E> List<E> readExcelWithModel(InputStream inputStream, Class<? extends BaseRowModel> clazz, ExcelTypeEnum excelTypeEnum) {
        ...
        ExcelReader excelReader = new ExcelReader(inputStream, excelTypeEnum, listener);
        //默认只有一列表头
     ...
     * @param data     导出数据
     */
    /*public static void writeSimpleExcel(String fileName, List<? extends BaseRowModel> data, HttpServletResponse response) {
        try {
            if (data.isEmpty()) {
                throw new ProException(EXCEPTION_EMPTY_EXCEL);
            }
            ...
            e.printStackTrace();
            throw new ProException(EXCEPTION_EXPORT_EXCEL);
        } finally {
            // 每次导出后清空数据
            data = null;
        }
     ...
     * @param data     导出数据
     */
    public static void writeExcel(String fileName, ExcelTypeEnum excelTypeEnum, List<? extends BaseRowModel> data, HttpServletResponse response) throws IOException {
        if (data.isEmpty()) {
            log.error("Excel为空！");
        }
        // 设置默认名称
        ...
        // 点击导出Excel按钮时，页面显示的默认名称
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String date = simpleDateFormat.format(new Date());
        // 防止中文乱码，XLSX大数据格式
        String newFileName = new String((fileName + "_" + date).getBytes(StandardCharsets.UTF_8), StandardCharsets.ISO_8859_1);
        ...
        List<? extends List<? extends BaseRowModel>> lists = Lists.partition(data, 50000);
        // 一页五万条，自动分页
        // 将文件输出
